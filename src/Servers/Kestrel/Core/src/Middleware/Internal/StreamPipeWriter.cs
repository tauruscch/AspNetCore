...
namespace System.IO.Pipelines
{
    internal class StreamPipeWriter : PipeWriter
    {
        ...
        {
            Logger.LogDebug("[{ConnectionId}]: Advance({bytes})", ConnectionId, bytes);

            if ((uint)bytes > (uint)_tailMemory.Length)
            {
                throw new Exception("ThrowHelper.ThrowArgumentOutOfRangeException(ExceptionArgument.bytes);");
            }
        ...
        {
            Logger.LogDebug("[{ConnectionId}]: GetMemory({size})", ConnectionId, sizeHint);

            if (_isCompleted)
            {
                throw new Exception("ThrowHelper.ThrowInvalidOperationException_NoWritingAllowed();");
            }

            if (sizeHint < 0)
            {
                throw new Exception("ThrowHelper.ThrowArgumentOutOfRangeException(ExceptionArgument.sizeHint);");
            }
        ...
        {
            Logger.LogDebug("[{ConnectionId}]: GetSpan({size})", ConnectionId, sizeHint);
            if (_isCompleted)
            {
                throw new Exception("ThrowHelper.ThrowInvalidOperationException_NoWritingAllowed();");
            }
        ...
        {
            Logger.LogDebug("[{ConnectionId}]: Begin Complete({exception})", ConnectionId, exception);

            if (_isCompleted)
            {
                Logger.LogDebug("[{ConnectionId}]: End Complete({exception}) short circuit", ConnectionId, exception);
                return;
            }
            ...
            Logger.LogDebug("[{ConnectionId}]: End Complete({exception})", ConnectionId, exception);
        }
        ...
        {
            Logger.LogDebug("[{ConnectionId}]: Begin FlushAsyncInternal()", ConnectionId);
            ...
            using (reg)
            {
                ...
                {
                    Logger.LogDebug("[{ConnectionId}]: End FlushAsyncInternal()", ConnectionId);
                }
            }
        }
    }
